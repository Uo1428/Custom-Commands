...
function slash(interaction, error) {
  ...
  client.channels.cache.get(client.config.Channels.CommandErrorLogs)?.send({
    embeds: [
      new EmbedBuilder()
        .setColor("DarkRed")
        .setTitle(`!{x} Error System [INTERACTION COMMANDS]`)
        ...
        .addFields([
          { name: "Guild", value: `\`\`\`yml\n${interaction.guild?.name} (${interaction.guild?.id})\`\`\``, inline: true },
          { name: "User", value: `\`\`\`yml\n${interaction.user.username} (${interaction.user.id})\`\`\``, inline: true }
        ])
    ]
  });
...
function msg(message, error) {
  ...
  client.channels.cache.get(client.config.Channels.CommandErrorLogs)?.send({
    embeds: [
      new EmbedBuilder()
        .setColor("DarkRed")
        .setTitle(`!{x} Error System [Message COMMANDS]`)
        .setDescription(`_An error has occured_.\n\n**Error Code:** \`${error.name}\`\n**Error Message:** \`${error.message}\`\n**Stack:** \`\`\`yml\n${error.stack}\`\`\``)
